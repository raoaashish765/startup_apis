...
app.post("/login", async (req, res) => {
  try {
    ...
    if (!isValidPassword(password)) {
      console.log(
        "Confirm password didn't match or password criteria not met......."
      );
      return res.status(400).json({
        message: "Confirm password didn't match or password criteria not met.",
      });
    }
    ...
    if (existingUsers[0].account_status != "active") {
      ...
      return res
        .status(400)
        .json({ message: "Unfortunately your account is suspended." });
    }
    ...
    const token = jwt.sign({ the_username, usr_role }, secretKey, {
      expiresIn: "24h",
    });

    await pool.query("UPDATE users SET api_token = ? WHERE username = ?", [
      token,
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (
      permisioncheck.length !== 1 ||
      permisioncheck[0].role !== "superadmin"
    ) {
      console.log("token or role not match");
      return res.status(400).json({
        message:
          "Something Went Wrong, Your Account Is not Authorized! Please Log In Again",
      });
    }
    ...
    if (!/^[a-zA-Z\s]{5,}$/.test(the_name)) {
      console.log(
        "name should be only letters and spaces, minimum length 5......."
      );
      return res.status(400).json({
        message:
          "Name should be only letters and spaces with a minimum length of 5.",
      });
    }
    ...
    const [theres] = await pool.query("SELECT * FROM users WHERE user_id = ?", [
      result[0].insertId,
    ]);
...
app.post("/singleupdatepage/:id", async (req, res) => {
  ...
  try {
    ...
    await pool.query("UPDATE pages SET content = ? WHERE id = ?", [
      contentString,
      id,
    ]);
...
app.get("/singleeditsection/:id", async (req, res) => {
  ...
  try {
    const [rows] = await pool.query("SELECT * FROM sections WHERE id = ?", [
      id,
    ]);
...
app.post("/singleupdatesection/:id", async (req, res) => {
  ...
  try {
    const contentString = JSON.stringify(updatedData.content);
    console.log(contentString);
    await pool.query("UPDATE sections SET content = ? WHERE id = ?", [
      contentString,
      id,
    ]);
    res.status(200).json({ message: "Page data updated successfully" });
...
app.post("/api/pages/:pageid", async (req, res) => {
  ...
  try {
    const [auth] = await pool.query(
      "SELECT * FROM users WHERE username = ? AND api_token = ?",
    ...
    const thepage = await pool.query("SELECT * FROM pages WHERE id = ?", [
      pageid,
    ]);
    ...
    if (thepage[0][0].url !== req.body.datapg[4]) {
      const links = await pool.query(
        "SELECT GROUP_CONCAT(url SEPARATOR ',') AS urls FROM pages"
      );
      ...
      await pool.query("UPDATE pages SET url = ? WHERE id = ?", [
        req.body.datapg[4],
        pageid,
      ]);
    }
...
app.get("/api/dashdata", async (req, res) => {
  try {
    const totalVisitorQuery =
      "SELECT COUNT(*) AS totalVisitor FROM visitordata";
    const uniqueSessionsQuery =
      "SELECT COUNT(DISTINCT visitor_ip) AS uniqueSessions FROM visitordata";
...
app.get("/api/visitorchart", async (req, res) => {
  ...
  try {
    ...
    const todayData = await pool.query(
      `SELECT HOUR(date) AS hour, COUNT(*) AS users 
        FROM visitordata 
        WHERE DATE(date) >= ? 
        GROUP BY hour;
        `,
    ...
    const weekData = await pool.query(
      `SELECT DATE_FORMAT(date, '%Y-%m-%d') AS day, COUNT(*) AS users 
        FROM visitordata 
    ...
    const week = {
      ...
      labels: Array.from({ length: 7 }, (_, i) => {
        const indexdate = new Date(
          new Date(todayStartFormatted).getTime() - i * 24 * 60 * 60 * 1000
        )
          .toISOString()
          .split("T")[0];
        return indexdate;
    ...
    const monthData = await pool.query(
      `SELECT DATE_FORMAT(date, '%Y-%m') AS month, COUNT(*) AS users
        FROM visitordata
        WHERE date >= ? AND date < ?
        GROUP BY month;
    `,
...
app.post("/api/menuupdate", async (req, res) => {
  try {
    const contentAsJson = JSON.stringify(req.body.content);
    await pool.query("UPDATE menudata SET content = ? WHERE id = 1", [
      contentAsJson,
    ]);
    res.status(200).json({ message: "success" });
  } catch (error) {
...
app.post("/api/workcategorydel", async (req, res) => {
  try {
    ...
    await pool.query("DELETE FROM category_work WHERE id = ?", [delid]);
  ...
  try {
    ...
    const insertResult = await pool.query(
      ...
      [
        ...
        username,
        username,
        addedname[0].description,
        addedname[0].image1,
        addedname[0].image2,
        addedname[0].category,
      ]
    );
...
app.post("/api/workprojectdel", async (req, res) => {
  try {
    ...
    // delete the category from database
    await pool.query("DELETE FROM work_projects WHERE id = ?", [delid]);
...
app.post("/api/useraccsettingup", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      "SELECT * FROM users WHERE user_id = ? AND api_token = ?",
      [datatoupdate[0].user_id, accessToken]
    );
    ...
    if (rows[0].password === datatoupdate[0].password) {
      if (!datatoupdate[0].new_pass) {
        ...
        await pool.query(
          ...
          [
            datatoupdate[0].username,
            datatoupdate[0].email,
            datatoupdate[0].role,
            datatoupdate[0].profile_picture,
            datatoupdate[0].user_id,
            accessToken,
          ]
        );
      } else {
...
app.post("/api/addnewpage", async (req, res) => {
  try {
    ...
    const [the_template] = await pool.query(
      "SELECT * FROM templates WHERE page_name = ?",
      [selecttemp]
    );
...
app.post("/api/thepagesdel", async (req, res) => {
  try {
    ...
    // delete the category from database
    await pool.query("DELETE FROM pages WHERE id = ?", [delid]);
